...
public function single(Request $request)
{
    ...
    {
        $filteredData=substr($request->img, strpos($request->img, ",")+1);
        
        //Decode the string
        $unencodedData=base64_decode($filteredData);
        file_put_contents('public/signnatures/'.time().'av.png', $unencodedData);
        
        $add=ToolboxSignature::where('file_id',$request->f_id)->where('user_id',Auth::user()->id)->first();
        $add->user_id=Auth::user()->id;
        $add->sign=$request->img;
        $add->folder_path = 'public/signnatures/'.time().'av.png';
...
//            rename($name, $rename);
    ...
    {
        return view('dashboard2.toolbox.seeting');
    }
    ...
    public function results()
    {
        $all = true;
        $projects=Projects::where('status',1)->where('company_id',Auth::user()->company_id)->get();
        $folders = Folder::where('type','toolbox')->where('company_id',Auth::user()->company_id)->pluck('id');
        $users=User::role('User')->where('company_id',Auth::user()->company_id)->get();
        // return $folders;
        if(Auth::user()->hasRole('Admin')){
            $toolboxes = Toolbox::whereIn('folder_id', $folders)->with('signatures.user')->get();
        }else{
            $toolboxes = ManagerHasToolbox::where('manager_id',Auth::user()->id)->whereIn('folder_id',$folders)->with('signatures.user')->get();
        }
        return view('dashboard2.toolbox.toolbox-results',compact('toolboxes','users', 'all', 'projects'));
    }

    public function results_individual($employe_id)
    {
        $all = false;
        $projects=Projects::where('status',1)->where('company_id',Auth::user()->company_id)->get();
        $users=User::role('User')->where('company_id',Auth::user()->company_id)->get();
        $signatures = ToolboxSignature::where(['user_id'=>$employe_id])->get();
        return view('dashboard2.toolbox.toolbox-results',compact('signatures','users','employe_id', 'all', 'projects'));
    }

    public function results_projects($project_id)
    {
        $all = false;
        $projects=Projects::where('status',1)->where('company_id',Auth::user()->company_id)->get();
        $users=User::role('User')->where('company_id',Auth::user()->company_id)->get();
        $workers=Worker::where('job_no',$project_id)->pluck('user_id');
        $signatures = ToolboxSignature::whereIn('user_id',$workers)->get();
        return view('dashboard2.toolbox.toolbox-results',compact('signatures','users','project_id', 'all', 'projects'));
